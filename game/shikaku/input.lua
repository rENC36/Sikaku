...
local INVALID_COLOR = vmath.vector4(1.0, 0.12, 0.12, 1)
	...
	if not info.valid then
		...
		elseif info.reason == "overlap" then
			...
			if info.overlapping_rect then
				print("Overlaps with:",
				info.overlapping_rect.x,
				info.overlapping_rect.y,
				info.overlapping_rect.width,
				info.overlapping_rect.height
			)
		end
	else
