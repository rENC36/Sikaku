...
local BlitzCFG     = require("data.config.blitz_config")
...
function BlitzGame.new()
	...
	self.score        = 0
	self.timer        = BlitzCFG.TIME.START
	self.grid_size    = BlitzCFG.GRID.START_SIZE
...
function BlitzGame:use_hint()
	...
	if ok then
		...
		print("[BlitzGame] Hint failed:", reason)
	end
	return ok, reason
end

function BlitzGame:solve_round()
	self.score = self.score + BlitzCFG.SCORE.BONUS
	self.timer = math.min(BlitzCFG.TIME.CAP, self.timer + BlitzCFG.TIME.BONUS)

	if self.score % BlitzCFG.GRID.GROW_EVERY == 0 and self.grid_size < BlitzCFG.GRID.MAX_SIZE then
